...
import by.gsu.epamlab.bll.ConnectDB;
import by.gsu.epamlab.bll.ConnectionException;
import by.gsu.epamlab.bll.IResultDAO;
import by.gsu.epamlab.bll.ResultImplBuffer;
import by.gsu.epamlab.bll.ResultsLoader;
import by.gsu.epamlab.bll.ThreadLoader;
	...
	public static void logic(ResultFactory resultFactory, String fileName) {
        ...
        try{
			try {
				...
				IResultDAO reader = resultFactory.getResultDaoFromFactory(resultFactory, fileName,buffer);
				
				ThreadLoader tl = new  ThreadLoader(buffer,reader);
				Thread th =  new Thread(tl);
				
				th.start();
				
				IResultDAO readerb = new ResultImplBuffer(buffer,reader);
				
				
				
					ResultsLoader.loadResults(readerb);
	...
	private static void printResults (ResultFactory resultFactory) throws ConnectionException {
		...
		try {
			...
			while(resultSet.next()) {
				...
				int mark = resultSet.getInt(Constans.MARK_IND);
